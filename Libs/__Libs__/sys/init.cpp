...
DLL_EXPORT table* sys_cmd(table* args[]) {
    if (args[0]->is_str) {
        return value(sys_cmd(__convert_to_string(args[0]->f_str_v)));
    }
...
DLL_EXPORT table* __write_file_content(table* args[]) {
    ...
    if (path->is_str && content->is_str) {
        return value(write_file_content(__convert_to_string(path->f_str_v), __convert_to_string(content->f_str_v)));
    } else {
        return value(-1);
    }
}
