...
MERCURY_API  __mer_core_lib_api__ void __io_cout(Mer_string str) {
    for (int i = 0; i < strlen(str); i++) {
        fwrite(to_char(str[i]), 1, 1, stdout);
    }
}
...
MERCURY_API __mer_core_lib_api__ void __builtin_io_write(stack *stk) {
    ...
    if (top->is_str) {
        ...
        if (!is_float(top->cval)) {
            Mer_string str = TO_STRING((int)top->cval);
            __io_cout(str);
            stk->s_table->table.pop_back();
        } else {
            Mer_string str = TO_STRING(top->cval);
            __io_cout(str);
            stk->s_table->table.pop_back();
        }
    }
