...
MERCURY_API __Mer_return_Code MVM_id(mAST_T *ast, __compiler_u &glb) {
  ...
  if (ast->type == ListStatement) return MVM_list(ast, glb);
  if (ast->type == ArrayExpression) return MVM_array(ast, glb);

  return NULL_CODE;
}

MERCURY_API __Mer_return_Code MVM_array(mAST_T *ast, __compiler_u &glb) {
  ...
  for (auto &item : ast->list) {
    __Mer_return_Code _code = MVM_id(item, glb);
  ...
  #ifdef SYSTEM_TEST
  cout << "[compiler.cpp] [MVM_array] [pass]" << endl;
  #endif
  return result;
}

MERCURY_API __Mer_return_Code MVM_list(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
  ...
  #ifdef SYSTEM_TEST
  cout << "[compiler.cpp] [MVM_list] [pass]" << endl;
  #endif
  return result;
}
...
MERCURY_API __Mer_return_Code MVM_return(mAST_T *ast, __compiler_u &glb) {
  ...
  ret_val = MVM_id(ast->return_v, glb);
...
MERCURY_API __Mer_return_Code MVM_program(mAST_T *ast) {
  #ifdef SYSTEM_TEST
  cout << "[compiler.cpp] [MVM_program] [start]" << endl;
  #endif
...
MERCURY_API __Mer_return_Code MVM_string_var(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
  ...
  __Mer_return_Code str_val = MVM_id(ast->str_var_value, glb);
  GLOBAL_TABLE.push_back(CREAT_GLOBAL_TABLE(glb.address, name, real_name));
  ...
  #ifdef SYSTEM_TEST
  cout << "[compiler.cpp] [MVM_string_var] [pass]" << endl;
  #endif
