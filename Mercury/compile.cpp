...
MERCURY_API __Mer_return_Code MVM_list(mAST_T *ast, __compiler_u &glb) {
  ...
  if (ast->type != ListStatement) {
    MerDebug_system_error(COMPILER_ERROR, "Invalid creating list");
  }
...
MERCURY_API __Mer_return_Code MVM_program(mAST_T *ast) {
  ...
  GLOBAL_TABLE.push_back(CREAT_GLOBAL_TABLE(IO_WRITE_ADDRESS, "write", "write"));
  GLOBAL_TABLE.push_back(CREAT_GLOBAL_TABLE(EVAL_ADDRESS, "eval", "eval"));
