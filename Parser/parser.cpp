...
mAST_T *MerParser_parse_primary_expression(mParser_T *parser)
{
    ...
    if (parser->token->tok == EOF_T || parser->token->tok == DO_T || parser->token->tok == EOL_T || parser->token->tok == LESS || parser->token->tok == GREATER || parser->token->tok == STRING || parser->token->tok == FUNCTION_CALL) return NULL;
