...
mAST_T *MerParser_parse_if_statement(mParser_T *parser)
{
    ...
    if (!comp)
    {
        mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing condition in if statement", parser->lexer->inp, "main.mer");
        return NULL;
    }
    node->comp = comp;
    ...
    if (parser->token->tok == THEN)
    {
        ...
        while (parser->token->tok != END_T)
        {
            ...
            {
                mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing body in if statement", parser->lexer->inp, "main.mer");
            }
        }
    }
    else
    {
        mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing 'THEN' keyword in if statement", parser->lexer->inp, "main.mer");
        return NULL;
    }
    ...
    if (parser->token->tok == ELSE) {
        ...
        while (parser->token->tok != END_T)
        {
            ...
            {
                mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing body in else statement", parser->lexer->inp, "main.mer");
            }
        }
...
mAST_T *MerParser_parse_while_statement(mParser_T *parser) {
    ...
    if (!comp)
    {
        mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing condition in while statement", parser->lexer->inp, "main.mer");
        return NULL;
    }
    ...
    if (parser->token->tok == DO_T) {
        ...
        while (parser->token->tok != END_T)
        {
            ...
            {
                mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing body in while statement", parser->lexer->inp, "main.mer");
            }
        }
        parser->token = _MerLexer_get_next_tok(parser->lexer);
        return node;
    } else {
        mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing 'DO' keyword in while statement", parser->lexer->inp, "main.mer");
        return NULL;
    }
}
...
mAST_T *MerParser_parse_primary_expression(mParser_T *parser)
{
    ...
    if (is_tok_literal(parser->token->tok))
