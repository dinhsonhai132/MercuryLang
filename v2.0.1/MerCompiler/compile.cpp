...
char *MVM_compile_ast_binary_expression(AST_node *ast)
{
    ...
    else if (ast->op == MINUS) oper = "BINARY_SUB";
    else if (ast->op == TIME) oper = "BINARY_MUL";
    else if (ast->op == DIV) oper = "BINARY_DIV";
    else return nullptr;
...
int main(int argc, char *argv[]) {
    ...
    if (!infile) {
        cerr << "Error: Could not open source file " << source_file << endl;
        return 1;
    }
    ...
    char *bytecode = MVM_compiler(source);
    if (!bytecode) {
        cerr << "Error: Compilation failed" << endl;
        return 1;
    }

    string output_file = source_file.substr(0, source_file.find_last_of('.')) + ".merc";
    ...
    cout << "Compilation successful. Output written to " << output_file << endl;
    return 0;
}