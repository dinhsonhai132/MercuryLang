...
__Mer_return_Code MVM_id(mAST_T *ast, __compiler_u &glb) {
...
__Mer_return_Code MVM_literal(mAST_T *ast, __compiler_u &glb) {
...
__Mer_return_Code MVM_binary_expression(mAST_T *ast, __compiler_u &glb) {
...
__Mer_return_Code MVM_function_call(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
...
__Mer_return_Code MVM_if(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
  __Mer_return_Code comp = NULL_CODE;
  __Mer_return_Code if_body = NULL_CODE;
  if (ast->comp->type == ComparisonExpression) comp = MVM_id(ast->comp, glb);
  else mer_error(SYSTEM_ERROR, "Syntax error: Invalid condition in if statement");
  ...
  PUSH(result, CJUMP_IF_TRUE);
  PUSH(result, glb.address);
  PUSH_TO_OUTCODE(result, CMAKE_BLOCK);
  PUSH_TO_OUTCODE(result, glb.address);
  INSERT_VEC_TO_OUTCODE(result, if_body);
  PUSH_TO_OUTCODE(result, CEND_BLOCK);

  return result;
}

__Mer_return_Code MVM_let(mAST_T *ast, __compiler_u &glb) {
...
__Mer_return_Code MVM_return(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
  PUSH(result, CRETURN);
  return result;
}

__Mer_return_Code MVM_identifier(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
  Mer_uint8_t address;
  Mer_bool found = false;
  ...
  if (!found) {
    mer_error(SYSTEM_ERROR, "Variable not found: " + ast->string_iden);
    return NULL_CODE;
  }
...
__Mer_return_Code MVM_program(mAST_T *ast) {
  __compiler_u glb;
  return MVM_id_expression_statment(ast, glb);
}

__Mer_return_Code MVM_id_expression_statment(mAST_T *ast, __compiler_u &glb) {
...
__Mer_return_Code MVM_function(mAST_T *ast, __compiler_u &glb) {
  __Mer_return_Code result = NULL_CODE;
...
__Mer_return_Code MVM_comparison_expression(mAST_T *ast, __compiler_u &glb) {
