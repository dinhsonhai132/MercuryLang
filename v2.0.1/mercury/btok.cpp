...
ctoken MerByteCodeTokenize_tokenize_code(vector<uint8_t> code) {
  ...
  while (global_code_idx < code.size()) {
    ...
    if (c == CPUSH_FLOAT) {
      ...
      token.bcode.push_back(c);
      token.ctokens.push_back(token);
    } else if (c == CLESS || c == CGREATER || c == CLESS_EQUAL || c == CGREATER_EQUAL || c == CEQUAL || c == CNOT_EQUAL) {
      token.code = c;
      switch (c) {
        case CLESS: token.scode = "CLESS"; break;
        case CGREATER: token.scode = "CGREATER"; break;
        case CLESS_EQUAL: token.scode = "CLESS_EQUAL"; break;
        case CGREATER_EQUAL: token.scode = "CGREATER_EQUAL"; break;
        case CEQUAL: token.scode = "CEQUAL"; break;
        case CNOT_EQUAL: token.scode = "CNOT_EQUAL"; break;
      }
      token.cbyte += 0x0001;
      token.cline += 0x0001;
      token.rval = 0.0;
      token.bcode.push_back(c);
      token.ctokens.push_back(token);
    } else if (c == CPOP_NORMAL_MODE) {
      token.code = c;
      token.scode = "POP_NORMAL_MODE";
      token.cbyte += 0x0001;
      token.cline += 0x0001;
      token.rval = 0.0;
      token.bcode.push_back(c);
