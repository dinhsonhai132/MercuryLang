...
vector<uint8_t> MerCompiler_compile_ast(mAST_T *ast, bc_mode_tok &prog) {
    ...
    if (ast->type == IfStatement) return MerCompiler_compile_ast_if_statement(ast, prog);
    return {0x0000};
}

vector<uint8_t> MerCompiler_compile_ast_if_statement(mAST_T *ast, bc_mode_tok &prog) {
  return {0x0};
}
...
mObject_T *MerCompiler_compile(mAST_T *ast, mCode_T &code) {
  if (!ast) return NULL_OBJECT_PTR;

  if (ast->type == Program) return MerCompiler_compile_program(ast, code);
  if (ast->type == LetStatement) return MerCompiler_compile_let_statement(ast, code);
  if (ast->type == IfStatement) return MerCompiler_compile_if_statement(ast, code);
  if (ast->type == FunctionStatement) return MerCompiler_compile_function(ast, code);
  if (ast->type == FunctionCallExpression) return MerCompiler_compile_function_call(ast, code);
  if (ast->type == ReturnStatement) return MerCompiler_compile_return_statement(ast, code);
  if (ast->type == BinaryExpression) return MerCompiler_compile_binary_expression(ast, code);
  if (ast->type == Identifier_) return MerCompiler_compile_identifer(ast, code);
  if (ast->type == Literal) return MerCompiler_compile_literal(ast, code);
  
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_binary_expression(mAST_T *ast, mCode_T &code) {
  mObject_T *left = MerCompiler_compile(ast->left, code);
  mObject_T *right = MerCompiler_compile(ast->right, code);
  
  if (!left || !right) return NULL_OBJECT_PTR;
...
mObject_T *MerCompiler_compile_literal(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_program(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_identifer(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_let_statement(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_if_statement(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_function_call(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_return_statement(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_function(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}

mObject_T *MerCompiler_compile_function_body(mAST_T *ast, mCode_T &code) {
  return NULL_OBJECT_PTR;
}
