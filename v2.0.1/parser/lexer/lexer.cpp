...
int _MerLexer_free(mLexer_T *lexer) {
    if (lexer->buf) free((char *)lexer->buf);
    if (lexer->inp) free((char *)lexer->inp);
    if (lexer->iden) free((char *)lexer->iden);
    if (lexer->str) free((char *)lexer->str);
    if (lexer->com) free((char *)lexer->com);
    if (lexer->sytx) free((char *)lexer->sytx);
    free(lexer);
    return 0;
}
