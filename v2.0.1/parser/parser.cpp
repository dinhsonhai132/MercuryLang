...
mAST_T *MerParser_parse(mParser_T *parser) {;
    ...
    if (parser->token->tok == CLASS) return MerParser_parse_class_statement(parser);
...
mAST_T *MerParser_parse_primary_expression(mParser_T *parser) {
    ...
    if (parser->token->tok == EOF_T || parser->token->tok == END_T || parser->token->tok == DO_T || parser->token->tok == EOL_T) return NULL;
