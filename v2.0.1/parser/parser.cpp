...
mAST_T *MerParser_parse(mParser_T *parser) {;
    ...
    if (parser->token->tok == IF) return MerParser_parse_if_statement(parser);
...
mAST_T *MerParser_parse_if_statement(mParser_T *parser) {
    ...
    if (parser->token->tok == THEN) {
        parser->token = _MerLexer_get_next_tok(parser->lexer);
        while (parser->token->tok != END_T && parser->token->tok != ELSE) {
    ...
    if (parser->token->tok == END_T) {
        parser->token = _MerLexer_get_next_tok(parser->lexer);
    } else {
        mer_run_time_error(to_string(parser->lexer->line), "Syntax error: Missing 'END' keyword in if statement", parser->lexer->inp, "main.mer");
        return NULL;
    }
