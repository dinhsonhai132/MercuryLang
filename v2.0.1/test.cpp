...
void print_ast(AST_node *node, int indent = 0) {
    ...
    if (node->type == BinaryExpression) {
        ...
        cout << string(indent, ' ') << "{ type: LetStatement, name: " << node->var_name << ",\n";
        cout << string(indent + 2, ' ') << "value: ";
        print_ast(node->var_value, indent + 2);
        cout << string(indent, ' ') << "}" << endl;
        return;
    } else if (node->type == FunctionCallExpression) {
        cout << string(indent, ' ') << "{ type: FunctionCallExpression, name: " << node->func_call << ",\n";
    ...
    cout << string(indent, ' ') << "{ type: " << node->type << ", tok: " << node->tok << ", value: " << node->value << " }\n";
