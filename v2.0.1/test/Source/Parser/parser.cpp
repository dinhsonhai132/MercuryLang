...
AST_node *Parser::MerParser_print_statement() {
    ...
    cur_tok = get_ntok();
    state->poutput = MerParser_expression_statement();
    cur_tok = get_ntok();
    return state;
}

AST_node *Parser::MerParser_expression_statement() {
    AST_node *state = MerAST_make_parent(ExpressionStatement);
    state->children.push_back(MerParser_additive_expression());
    cur_tok = get_ntok();
    return state;
...
AST_node *Parser::MerParser_function_call() {
    AST_node *node = MerAST_make_parent(FunctionStatement);
    cur_tok = get_ntok();
    if (Is_tok_identifier(cur_tok.tok)) {
        node->func_name = cur_tok.name;
        cur_tok = get_ntok();
