...
int main(int argc, char* argv[]) {
    ...
    bool f_dash_dash_show = false;
    bool f_dash_dash_output = false;
    bool f_dash_dash_compiled = false;
    ...
    AST_node *ast = MerAST_make_AST(tokens);
    vector<uint8_t> code = MerCompiler_compile_ast_program(ast);
    ctoken token = MerByteCodeTokenize_tokenize_code(code);
    stack *stk = MerCompilerBytecode_evaluate_program(code);

    for (auto &s : stk->s_table->table) {
        cout << s->cval << endl;
    }
