...
char *MVM_compile_ast(AST_node *ast, vector<char*> &code_address)
{
    ...
    if (ast->type == CallExpression) return MVM_compile_ast_call_expression(ast, code_address);
...
char *MVM_compile_ast_call_expression(AST_node *ast, vector<char*> &code_address)
{
    if (!ast) return nullptr;
...
char *MVM_compile_ast_function_statement(AST_node *ast, vector<char*> &code_address) {
    ...
    sprintf(bytecoden, "LOAD_GLOBAL %s;\n", name);
