...
AST_node *Parser::MerParser_function_expression_statement() {
    ...
    if (cur_tok.tok == VARIABLE) {
        ...
        if (cur_tok.tok == LEFT_PAREN) {
            ...
            if (cur_tok.tok == DO_T) {
                cur_tok = get_ntok();
                while (cur_tok.tok != END_T) {
                    if (cur_tok.tok == LET) {
                        func->body.push_back(MerParser_let_statement());
                    } else if (cur_tok.tok == RETURN) {
                        func->body.push_back(MerParser_return_statement());
                    } else if (cur_tok.tok == FUNCTION_CALL) {
                        func->body.push_back(MerParser_function_call_statement());
                    } else if (cur_tok.tok == FUNCTION) {
                        func->body.push_back(MerParser_function_expression_statement());
                    } else {
                        func->body.push_back(MerParser_additive_expression());
                    }
                    cur_tok = get_ctok();
                }
                cur_tok = get_ntok();
            } else {
                cout << "Error: Function body expression not found!\n";
                return func;
            }
        } else {
            cout << "Error: Function arguments not found!\n";
            return func;
        }
    } else {
        cout << "Error: Function name not found!\n";
        return func;
    }
